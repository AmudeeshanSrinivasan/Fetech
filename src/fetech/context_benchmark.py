    ...
    *,
    ...
    async def evaluate(task: ContextBenchmarkTask) -> BenchmarkTaskResult:
        ...
        baseline_paths = frozenset(
            path
            for need in task.expected_needs
            for path in corpus.paths_by_need[need]
        )
        baseline_tokens = sum(corpus.tokens_by_path[path] for path in baseline_paths)
        matched = tuple(
            expectation.id
...
def _build_baseline_corpus(
    ...
    all_paths = frozenset(path for paths in paths_by_need.values() for path in paths)
