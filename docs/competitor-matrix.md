...
Fetech baseline: **v0.3.0a0**, commit `7510ec9a8645`
...
[v0.3 conformance report](v0.3-authentication-foundation.md), and
[Apache-2.0 license](../LICENSE). At this baseline, 119 of 155 catalogue entries have v0.1-v0.3
implementation paths. Document, media, OCR, snapshot, and archive category closure remains v0.4
work, so those roadmap entries are not credited as shipped functionality here.

## Scope matrix

| Project | Officially checked scope | Material overlap with Fetech v0.3 | Different or not established in the checked scope | Upstream license statement |
| --- | --- | --- | --- | --- |
| **Fetech v0.3.0a0** | A Python SDK, CLI, REST daemon, and MCP surface over deterministic planning, policy checks, HTTP/browser/crawl acquisition, origin-scoped authentication, structured API/feed normalization, typed artifacts, CAS, cache isolation, and an append-only event ledger. The current [v0.3 report](v0.3-authentication-foundation.md) closes 23/23 v0.3 paths and the [catalogue](capability-catalog.md) records 119/155 cumulative paths. | Reference baseline. Its distinguishing design target is one policy/result/provenance contract across independently implemented acquisition engines. | The remaining 36 document, media, cache/snapshot, and archive capabilities are v0.4 work. The alpha has not demonstrated competitor-scale throughput or production maturity. | [Apache-2.0](../LICENSE). |
...
- **Fetech:** [capability catalogue](capability-catalog.md),
  [v0.3 conformance](v0.3-authentication-foundation.md), [architecture](architecture.md), and
  [license](../LICENSE).
