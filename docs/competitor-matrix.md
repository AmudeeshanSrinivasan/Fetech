...
Development scope checked here: **unreleased v0.4 implementation**
...
[v0.4 development conformance report](v0.4-conformance.md), and
[Apache-2.0 license](../LICENSE). The tagged release contains 119/155 paths. The local v0.4
development state adds 36 importable native or typed optional paths for 155/155, but it is not
...
| --- | --- | --- | --- | --- |
| **Fetech v0.3.0a0 + unreleased v0.4 development state** | A Python SDK, CLI, REST daemon, and MCP surface over deterministic planning, policy checks, HTTP/browser/crawl acquisition, origin-scoped authentication, structured API/feed normalization, bounded document/archive subprocesses, bounded media tools/providers, built-in bounded yt-dlp metadata and exact-host Wayback connectors, immutable validated snapshots, typed artifacts, CAS, and an append-only event ledger. The [v0.3 report](v0.3-authentication-foundation.md) records 119 published paths; [v0.4 conformance](v0.4-conformance.md) records 36 development paths. | Reference baseline. Its distinguishing design target is one policy/result/provenance contract across independently implemented acquisition engines. | v0.4 is not tagged or published, and package metadata still says `0.3.0a0`. The preferred Docling path, POSIX startup deadline, shared primary-HTTP/Wayback admission, reference Linux containment backend for covered built-in offline workers, and source-tree plus development-wheel artifact-bound Docling smokes are implemented, but final clean tagged-wheel and release-commit Linux/systemd evidence, real artifact-bound optional-tool/service smoke, legal review of the exact model bundle and explicit NVIDIA/pypdfium2 LicenseRefs, brokered yt-dlp egress, final release artifacts, broader parser fuzzing, total storage lifecycle controls, and competitor-scale throughput remain unproven. | [Apache-2.0](../LICENSE). |
...
- **Fetech:** [capability catalogue](capability-catalog.md),
  ...
  [v0.4 development conformance](v0.4-conformance.md), [architecture](architecture.md), and
  [license](../LICENSE).
